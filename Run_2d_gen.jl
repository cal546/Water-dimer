...
# data input
data_ = readdlm("2d_data_0405.txt")
@show size(data_);
...
d = 2;
@show n_total;
X = [data_[i,1:d].*prepend!(ones(d-1),1) for i in 1:n_total]
y = data_[:,3];
...
F0 = FourierModel([randn(ComplexF64) for _ in 1:K], # ω values
    [.10*randn(d) for _ in 1:K]); # β values
